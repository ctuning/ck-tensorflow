...
def fill_detection(detection, file_name):
  try:
    ...
    print('Exception when reading data from {}'.format(detection['key'] + '.txt'))
    detection = None
  finally:
    return detection

def get_annotations(annotations, class_map):
  '''
  Get annotations from file to dictionary in memory

  Args:
    annotations: groundtruth (file with annotations)
    class_map: dictionary for classes, text id => number id 
...
def evaluate(res_dir, annotations, label_map_path):
  '''
  ...
  for file_index, file_name in enumerate(files):
    print('Loading detections: {} of {} ...'.format(file_index+1, len(files)))
