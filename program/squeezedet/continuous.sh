...
${CK_ENV_COMPILER_PYTHON_FILE} "../continuous.py" --image_dir="$IMAGES" --label_dir="$LABELS" --out_dir="$OUT_DIR" --checkpoint="$CHECKPOINT" --demo_net="$NET" --finisher_file="$FINISHER_FILE" --draw_boxes=$DB --skip_files_including="$SKIP_FILES_INCLUDING"
