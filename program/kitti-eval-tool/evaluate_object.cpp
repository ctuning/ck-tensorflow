...
vector<tDetection> loadDetections(string file_name, bool &compute_aos, bool &eval_car, bool &eval_pedestrian, bool &eval_cyclist, bool &success) {
  ...
  cout << "D: Loading detections from " << file_name << "..." << endl;
...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt,
  const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc,
  vector<int32_t> &ignored_det, int32_t &n_gt, DIFFICULTY difficulty){
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
  ...
  bool compute_fp, bool compute_aos=false, double thresh=0, bool debug=false){
...
bool eval_class (FILE *fp_det, FILE *fp_ap, FILE *fp_ori, CLASSES current_class,
  const vector< vector<tGroundtruth> > &groundtruth, const vector< vector<tDetection> > &detections,
  bool compute_aos, vector<double> &precision, vector<double> &aos, DIFFICULTY difficulty, int32_t N_TESTIMAGES) {
  ...
  cout << "D: FP thresholds for " << CLASS_NAMES[current_class] << ", difficulty=" << difficulty << ": ";
...
//  string result_dir     = "results/" + result_sha;
  ...
  for (int32_t i=0; i<N_TESTIMAGES; i++) {
    ...
    if (!gt_success) {
      ...
      cerr << "ERROR: Couldn't read ground truth: " << file_name << endl;
      return false;
    }
    if (!det_success) {
      mail->msg("ERROR: Couldn't read: %s", file_name);
      cerr << "ERROR: Couldn't read: " << file_name << endl;
      return false;
    }
  }
  mail->msg("  done.");
...
int32_t main (int32_t argc,char *argv[]) {
  cout << "KITTI evaluation tool" << endl;
  string gt_dir, image_set_filename, result_dir;
  int32_t N_TESTIMAGES;

  if (argc == 1) {
    cout << "Taking arguments from environment:" << endl;
    gt_dir = getenv("CK_KITTI_LABELS_DIR");
    cout << "CK_KITTI_LABELS_DIR=" << gt_dir << endl;
    image_set_filename = getenv("CK_IMAGE_LIST_FILE");
    cout << "CK_IMAGE_LIST_FILE=" << image_set_filename << endl;
    result_dir = getenv("CK_RESULTS_DIR");
    cout << "CK_RESULTS_DIR=" << result_dir << endl;
    N_TESTIMAGES = atoi(getenv("CK_IMAGE_COUNT"));
    cout << "CK_IMAGE_COUNT=" << N_TESTIMAGES << endl;
  }
  else {
  ...
  gt_dir             = ospj( kitti_dir, "label_2" ); // FIXME_MWM: should be part of input? configurable?
  image_set_filename = argv[2];
  result_dir         = argv[3];
