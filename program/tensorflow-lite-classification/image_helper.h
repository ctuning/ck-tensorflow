...
std::vector<float> resize_image(const ImageData& img_data,
                                int wanted_height, int wanted_width,
                                float input_mean = 128, float input_std = 128) {
  const int wanted_channels = img_data.channels;
  std::vector<float> out(wanted_height * wanted_width * wanted_channels);
  const size_t image_rowlen = img_data.width * img_data.channels;
  const float width_scale = static_cast<float>(img_data.width) / wanted_width;
  const float height_scale = static_cast<float>(img_data.height) / wanted_height;
  for (int y = 0; y < wanted_height; ++y) {
    const float in_y = y * height_scale;
    const int top_y_index = static_cast<int>(floorf(in_y));
    const int bottom_y_index = std::min(static_cast<int>(ceilf(in_y)), (img_data.height - 1));
    const float y_lerp = in_y - top_y_index; 
    const uint8_t* in_top_row = img_data.data.data() + (top_y_index * image_rowlen);
    const uint8_t* in_bottom_row = img_data.data.data() + (bottom_y_index * image_rowlen);
    float *out_row = out.data() + (y * wanted_width * wanted_channels);
    for (int x = 0; x < wanted_width; ++x) {
      const float in_x = x * width_scale;
      const int left_x_index = static_cast<int>(floorf(in_x));
      const int right_x_index = std::min(static_cast<int>(ceilf(in_x)), (img_data.width - 1));
